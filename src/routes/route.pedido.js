...
routerPedido.get("/pedidos", function(req, res){
...
routerPedido.post("/pedidos", function(req, res){

    let sql = `insert into pedido(id_usuario, nome, email, fone, endereco, bairro,
        cidade, uf, cep, total, dt_pedido) values(?,?,?,?,?,?,?,?,?,?, current_date)
        returning id_pedido`;

    let p = req.body;

    db.all(sql, [p.id_usuario, p.nome, p.email, p.fone, p.endereco, p.bairro,
        p.cidade, p.uf, p.cep, p.total], async function(err, rows) {
        if (err)
